...
export default {
  ...
  methods: {
    ...
    async handleSizeChange(val) {
      this.pagesize = val;
      const { data } = await this.$request.get("/Haixian", {
        params: {
    ...
    async handleDelete(index, row) {
      this.$confirm("此操作将永久删除该商品, 是否继续?", "提示", {
        ...
        .then(async () => {
          const result = await this.$request.delete("/Haixian/" + row._id);
    ...
    async Submit() {
      if (this.form.id) {
        const { data } = await this.$request.put("/Haixian/" + this.form.id, {
        ...
        await this.$request.post("/Haixian", {
          commodityCode: this.form.commodityCode,
          commodityName: this.form.commodityName,
          subTitle: this.form.subTitle,
          commodityPrice: this.form.commodityPrice,
